...
import { fmtPct, fmtNum, cn } from '@notera/ui/lib/utils'
import { Card } from '@notera/ui/components/ui/card'
import { Skeleton, EmptyState } from '@notera/ui/components/ui/skeleton'
...
export function Overview({ setTab }: { setTab: (t: TabId) => void }) {
  ...
  if (runs === null) return <div className="p-4 sm:p-6 space-y-5"><div className="grid grid-cols-2 lg:grid-cols-4 gap-4">{[0, 1, 2, 3].map((i) => <Skeleton key={i} className="h-24" />)}</div><div className="grid grid-cols-3 gap-4">{[0, 1, 2].map((i) => <Skeleton key={i} className="h-28" />)}</div></div>
