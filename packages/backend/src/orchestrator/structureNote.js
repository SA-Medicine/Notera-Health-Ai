...
import { emptyNote, emptyIssue, SCHEMA_VERSION } from '@notera/schema';
  ...
  if (llm) {
    try {
      const raw = await llm.generateContent(
        ...
        GEMINI_NOTE_SCHEMA,
        { timeoutMs: 60000, maxOutputTokens: Math.max(Number(process.env.GEMINI_MAX_OUTPUT_TOKENS) || 65536, 32768) }
      );
